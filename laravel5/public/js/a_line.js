...
$(document).ready(function(){
			...
			//推荐给好友
	$('a.detail-recomment').click(function(){
		var recomment_display = $('.recomment_wra').css('display');
		if(recomment_display=="block"){
			$('.recomment_wra').css('display','none');
			$(".recom-succeed").hide();
			$(".recomment_wra_mid").hide();
		}
		else{
			$('.recomment_wra').css('display','block');
			$(".recom-succeed").hide();
			$(".recomment_wra_mid").show();
		}
	});
			$('.close').click(function(){
		var clickHide = $(this).attr('data-hide');
	...
	if($('.time-price-one').length>0){
        ...
        var bid=pid=$tp.attr('data-pid');
			...
			//setTimeout("$('.quickBooker_select').trigger('change')",200);
			...
			$('.search_pp_calendar_d_box').on('click',function(){
				...
				var startDate=$(this).find('.search_pp_calendar_day_date').text();// 获取点击的日期
				
				if($(this).data('sellbale')){
					
					if(isSuperFree){
						
						superFreeRefresh(pid,startDate)
					}
					else{
						
						numAlert(pid,bid,startDate,flagTo(passType),flagTo(passSubType));
					}
				}
			});
			//pushTipInfo($tp);
...
function numAlert(productId,branchId,startDate,passType,passSubType){
	...
	var html='<div id="zhebi" style="display:none;"></div>'+'<div class="l_yuding" style="display:none;z-index:9990;">'+'<div class="pop_yuding_tit">开始预订</div>'+'<i class="l_close"></i>'+'<div class="pop_yuding_inner">'+'<form method="post" action="'+$("#phpurl").attr('pid')+'/order/order_login.php" id="nextForm" style="display:none;"></form>'+'<!-- -->'+'<div class="l_yuding_mid">'+'<ul id="list_num" class="l_pro_window"></ul>'+'<p class="l_introduce" style="display:none;">'+' 注：部分景区对儿童身高有特殊要求，请以公告为准。</p>'+'<p class="zoom"><a href="#" class="l_next">下一步</a></p>'+'</div>'+'</div>'+'</div>';
	var $body=$('body');
	...
	var inhtml='';
	 ...
	 $.ajax({  
             ...
             url:$("#phpurl").attr('pid')+"/add/ajax_select_v60.php?productId="+productId+"&choseDate="+startDate,  
             dataType: "jsonp",  
			...
			if(passType=='hotel'){
				...
				inhtml+='<li><label class="date-price-fc-label">您选择的出发日期：</label><b class="date-price-fc-margin"><span class="nowDate">'+startDate+'</span></b></li>';
				if(data.jsonMap.flag=='Y'){
					...
					if(item.branchName=='成人价')
					{
						crj=item.price;
						 inhtml+='<input type="hidden" name="crj" id="crj" value="'+crj+'" />'
					}
					if(item.branchName=='儿童价')
					{
						rtj=item.price;
						 inhtml+='<input type="hidden" name="rtj" id="rtj" value="'+rtj+'" />'
					}
...
function liveBtn(startDate,passType,passSubType,pid,bid){
	...
	$lyd.find('.l_next').on('click',function(){
		...
		if(len>0){
			...
			var params='';
				...
				if(inLen>0){
					...
					if(fbid=='106119')
					{
						...
						//formHtml+='<input type="text" name="_hidChenRen" value="'+jiage+'" />';
					formHtml+='<input type="text" name="person" value="'+fnum+'" />';
					}
			
				else
				{
				formHtml+='<input type="text" name="son" value="'+fnum+'" />';
				}
			};
		   ...
		   if(parseInt(startDate[1])<'10') startDate[1]='0'+startDate[1];
		   if(parseInt(startDate[2])<'10') startDate[2]='0'+startDate[2];

		    startDate= startDate[0]+'-'+startDate[1]+'-'+ startDate[2];
	        formHtml+='<input type="text" name="pid"  id="pid" value="'+pid+'" />';
			 formHtml+='<input type="text" name="yuding"  id="yuding" value="line" />';
			  formHtml+='<input type="text" name="U7188Date"  id="U7188Date" value="'+startDate+'" />';
			 formHtml+='<input type="text" name="_hidChenRen"  id="_hidChenRen" value="'+$('#crj').val()+'" />';
			  formHtml+='<input type="text" name="_hidErTong"  id="_hidErTong" value="'+$('#rtj').val()+'" />';
			...
			/*
			$.ajax({
				url:""+params,dataType:"json",async:true,success:function(data){
...
function flagTo(code){
	switch(code){
		case'TICKET':return'ticket';
		case'':return'ticket';
		case'HOTEL':return'hotel';
		case'SINGLE_ROOM':return'singleRoom';
		case'HOTEL_SUIT':return'hotelSuit'
	}
};
...
function Order() {
    ...
    var godate =document.getElementById('selectTimePrice').value;
	 //var godate = $("#" + obj + " input.pTime").val();
if(parseInt(godate)=='1' || godate=='null' || godate=='undefined')
{
    ...
    if (godate != null && godate != "点击选择出发日期") {
        if ($("#param61423").val() > 0) {
            ...
            var person = $("#param61423").val(); //成人数
            var son = $("#param87148").val(); //儿童数
            ...
            //editWidget("/order_login/" + UzaiStartDate + "/" + person + "/" + son + "/" + _hidChenRen + "/" + _hidErTong + "/" + pid + ".html");
             ...
             editWidget(phpurl+"/order/order_login.php?pid="+pid+"&U7188Date="+godate+"&person="+person+"&son="+son+"&_hidChenRen="+_hidChenRen+"&_hidErTong="+_hidErTong+"&yuding=line");
        }
        else {
            alert("成人数须大于0！");
...
function chufrq(da)
{
	    ...
	    var productId=$("div#page").attr("pid"); 
	 ...
	 $.ajax({  
             ...
             async: false,  
             //url: "http://test/jsonp.php",  
             url:$("#phpurl").attr('pid')+"/add/ajax_select_v60.php?productId="+productId+"&choseDate="+da,  
             dataType: "jsonp",  
             jsonp: "callback",//传递给请求处理程序或页面的，用以获得jsonp回调函数名的参数名(一般默认为:callback)  
            
             success: function(data){ 
		var jiage=data.jsonMap.price[0].price.split('.');
		
        $("#_hidChenRen").val(jiage[0]);
		var jiage1=data.jsonMap.price[1].price.split('.');
		
		
		
		
		if(jiage1[0]>0) $("#_hidErTong").val(jiage1[0]);
		}
	});
