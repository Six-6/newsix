...
return tmpl||'';}
var rightCommon=rightCommon||[];var rightCommonUl="";var rightCommonrcTop="";var rightCommonrcMid="";var rightCommonrcBtm="";var rightCommonrcLastBtm="";var rihtCommonPhone="";rightCommon={init:function(){this.initToolBar();var topImfor=this.getAdImg()
...
+"</div>"
+"</div><%}%>"
+"</li><%}%>";var listImg=_.template(adImg,data.adArea);return getUnderscoreCompiledTemplate(listImg,data.adArea);}else{return"";}},getApp:function(){if(this.noProduct()){var app="<% if(data.appArea){ %><li style='height:42px;margin-top:20px;cursor:pointer;'>"
...
+"</a><%}%>"
+"</div>"
+"</li>";return getUnderscoreCompiledTemplate(_.template(orderList,data.myOrder),data.myOrder);}else{return"";}},myListInfor:function(){var listNum="<% if(data.islogin == 1 && data.myMessage.sum>0){ _.each(prd,function(name){ %><li>"
...
+"</div>"
+"</div>"
+"</li>";return backTop;}};;var checkIsLogin=checkIsLogin||[];var compName=compName||[];var clearTim=clearTim||"";var checkLogin={init:function(){this.submitInfor();},rcUserName:"",rcPassWord:"",rcVerCode:"",reTemp:1,doAddToCollect:function(){var __this=this;var doAddToCollect=$("#favorite_id_rc");doAddToCollect.click(function(){_gaq.push(['_trackEvent','[产品页]','[浮动层点击]','[加入收藏]']);var addbtn=$("#favorite_id_rc");if(addbtn.hasClass("on")){}
var addbtn_top=addbtn.offset().top;$("body,html").animate({"scrollTop":(addbtn_top-200)});__this.clickAddScroll();});},clickAddScroll:function(){var __this=this;var userId=$("#userId").attr("user_id"),routeId=$("#route_id_tmp").val();if(userId>0){var url="";var days="";var info="";if(routeId>5000000&&routeId<8000000){url="/main.php?do=user_route_collect&flag=collectInsert&isNewTour=1";days=$('#journeyDays').val();info={userId:userId,routeId:routeId,days:days};}else{url="/main.php?do=user_route_collect&flag=collectInsert";info={userId:userId,routeId:routeId};}
$.get(url,info,function(data){if(data==1){__this.disClick();$('#collcet_id').show();$('#delete_collcet_id').hide();return;}else if(data==2){$('#delete_collcet_id').show();$('#collcet_id').hide();$("#favorite_id_rc").removeClass("al_collect").addClass("collect");$(".change_price").children('div').removeClass('change_price_con').css('display','none');$('.start_subscribe').addClass('change_price_con');$('.change_plan_date').addClass('change_price_date');$('.change_price_list .subscribe_info').hide();$('.select_date1').find('.s_con').text('请选择出发日期');$('.select_date2').find('.s_con').text('请选择出发日期');$('.email').val('');return;}else if(data==3){alert('添加收藏失败!');return;}else if(data==4){alert('删除收藏失败!');return;}});}else{$('#login_id').show();return;}},disClick:function(){$("#favorite_id_rc").addClass("al_collect").removeClass("collect");;},addToDoCom:function(){var __this=this;$("#addToComp").click(function(){__this.addToCompareList();});},showCompareList:function(){var __this=this;var routeIds=this.getCookie("_compare");if(routeIds!=''){__this.get_compare_info(routeIds);}},showCompareBox:function(){var rightCommon=$("#rightCommon");var __w_width=$(window).width();if(__w_width<=1280){rightCommon.addClass("mouse_over");}
...
var comPareList=document.getElementById("comPareList");var base_url="http://www.tuniu.com";var url=base_url+'/yii.php?r=toursAjax/getInfo/routeIds/'+routeIds+'&jsoncallback=?';$.getJSON(url,function(r){compName=r;var compModuel=[{"id":"","showName":""}]
...
data=$.extend(true,{},default_data,save_data);rightCommon.init();checkLogin.init();checkLogin.showCompareList();checkLogin.addToDoCom();checkLogin.doAddToCollect();checkLogin.refreshLoginState();getLoginInfor.init();getData.getNameFormCookie(data.islogin);getData.addLive800();getData.backToTop();getData.selectTag();getData.createVCode();getData.compHeig();getData.smallWindow();getData.bindEvent();getData.leftRightSlide();if(save_data&&save_data.islogin==1){getData.getMsg();}};var getData=getData||[];getData={init:function(){this.getAllData();this.compHeig();},getAllData:function(){if(!$("#page_type").val())return false;var url='http://www.tuniu.com/api/sidebar/tools/'+$("#page_type").val()+"&js_callback=cjCallback";$.ajax({url:url,type:"get",async:true,dataType:"jsonp",jsonp:"js_callback"})},getMsg:function(){getData.msgNumEle=$('#J_RightCommonMsgNum');getData.msgListEle=$('#J_RightCommonMsgList');setTimeout(function(){getData.getMsgData();},loopDuration);},getMsgData:function(){$.get('http://www.tuniu.com/u/msg/unread/',{},function(data){var listHtml;if(data&&data.sum){listHtml=_.template(rightCommon.myMsgTemplate(),{myMessage:data});getData.msgListEle.html(listHtml);getData.msgNumEle.text(data.sum).show();}else{getData.msgListEle.html('暂无新消息');getData.msgNumEle.hide().text(0);}
