! function(a, b) {
	...
	var c, d, e = "",
		f = {
			getPath: function() {
				var a = document.scripts || document.getElementsByTagName('script'),
					b = a[a.length - 1].src;
	...
	h.pt = h.prototype, h.pt.config = {
		...
		switch (d.type) {
			case 0:
				d.title || (d.area = ["384px", "125px"]), c(".xubox_dialog")[0] && layer.close(c(".xubox_dialog").parents("." + g[0]).attr("times"));
				break;
			case 1:
		...
		var b = this,
			...
			j = i.find(g[2]);
		switch (e.title ? 0 === e.type && layer.ie6 && j.css({
			width: i.outerWidth()
		}) : 4 !== e.type && i.find(".xubox_close").addClass("xubox_close0"), i.attr({
			type: f.type[e.type]
			...
			case 0:
				i.find(g[5]).css({
					...
					paddingTop: 50 + j.outerHeight()
				}) : 7 === e.dialog.type ? (i.find(".xubox_msgico").css({
					top: 45,
					left: 50
				}), i.find(g[3]).css({
					marginTop: 50,
		...
		b.autoArea(a), e.fadeIn && (f.fade(i, e.fadeIn, 1), f.fade(c("#xubox_shade" + a), e.fadeIn, e.shade[0])), e.fix && "" === e.offset[0] && !e.shift && d.on("resize", function() {
			i.css({
